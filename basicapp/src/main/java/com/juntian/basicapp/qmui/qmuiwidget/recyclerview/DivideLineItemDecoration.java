package com.juntian.basicapp.qmui.qmuiwidget.recyclerview;
...
public class DivideLineItemDecoration extends RecyclerView.ItemDecoration {
     ...
     * @param mItemSize 线宽
     */
    public DivideLineItemDecoration(Context context, int color, int mItemSize,int COMPLEX_UNIT,boolean nothing) {
        this.mSize = mItemSize;
        mSize = (int) TypedValue.applyDimension(COMPLEX_UNIT, mItemSize, context.getResources().getDisplayMetrics());
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setColor(color);
        mPaint.setStyle(Paint.Style.FILL);//设置填充
        mOrientation = VERTICAL_LIST;   //默认是垂直方向
